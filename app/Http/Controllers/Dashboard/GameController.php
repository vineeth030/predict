...
private function assignRank()
{
    ...
    foreach ($companyGroupIds as $companyGroupId) {
        ...
        foreach ($users as $user) {
            $userModel = User::find($user->id);
            $userModel->old_rank = $userModel->new_rank;  // Save the current new rank as old rank

            // Check if the current user's points are the same as the previous user's points
